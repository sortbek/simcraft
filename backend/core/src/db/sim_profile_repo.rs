    ...
    pub async fn list(&self) -> Result<Vec<SavedSimProfile>, sqlx::Error> {
        match &self.backend {
            ProfileBackend::Database(pool) => {
                let rows = sqlx::query(
                    "SELECT id, name, data, created_at, updated_at FROM sim_profiles ORDER BY updated_at DESC",
                )
                .fetch_all(pool)
                .await?;
                Ok(rows
                    .iter()
                    .map(|r| SavedSimProfile {
                        id: r.get("id"),
                        name: r.get("name"),
                        data: r.get("data"),
        ...
        match id {
            Some(id) => match &self.backend {
                ProfileBackend::Database(pool) => {
                    ...
                    let row = sqlx::query(
                        "SELECT id, name, data, created_at, updated_at FROM sim_profiles WHERE id = $1",
                    )
                    .bind(id)
                    .fetch_one(pool)
                    .await?;
                    Ok(Some(SavedSimProfile {
                        id: row.get("id"),
                        name: row.get("name"),
                        data: row.get("data"),
            ...
            None => {
                ...
                match &self.backend {
                    ProfileBackend::Database(pool) => {
                        sqlx::query(
                            "INSERT INTO sim_profiles (id, name, data, created_at, updated_at) VALUES ($1, $2, $3, $4, $5)",
                        )
                        .bind(&profile.id)
                        .bind(&profile.name)
                        .bind(&profile.data)
...
mod tests {
    ...
    /// One in-memory SQLite DB for the whole pool: max_connections(1), matching
    /// production (`Database::connect` also pins SQLite to a single connection).
