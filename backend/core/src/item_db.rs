...
pub fn load(data_dir: &Path) {
    ...
    if items_path.exists() {
        ...
        let map: HashMap<u64, Value> = data.into_iter()
            .filter_map(|v| { let id = v.get("id")?.as_u64()?; Some((id, v)) })
            .collect();
    ...
    if enchants_path.exists() {
        ...
        let by_id: HashMap<u64, Value> = data.iter()
            .filter_map(|v| { let id = v.get("id")?.as_u64()?; Some((id, v.clone())) })
            .collect();
        let by_item_id: HashMap<u64, Value> = data.into_iter()
            .filter_map(|v| { let item_id = v.get("itemId")?.as_u64()?; Some((item_id, v)) })
            .collect();
    ...
    if bonuses_path.exists() {
        ...
        let map: HashMap<u64, Value> = raw.into_iter()
            .filter_map(|(k, v)| { let id = k.parse::<u64>().ok()?; Some((id, v)) })
            .collect();
...
pub fn resolve_bonuses(bonus_ids: &[u64]) -> Value {
    ...
    for bid in bonus_ids {
        if let Some(bonus) = bonuses().get(bid) {
            if let Some(q) = bonus.get("quality") { result["quality"] = q.clone(); }
            if let Some(il) = bonus.get("itemLevel").and_then(|il| il.get("amount")) { result["ilevel"] = il.clone(); }
            if let Some(tag) = bonus.get("tag") { result["tag"] = tag.clone(); }
            if let Some(socket) = bonus.get("socket") { result["sockets"] = socket.clone(); }
            if let Some(upgrade) = bonus.get("upgrade").and_then(|u| u.get("fullName")) { result["upgrade"] = upgrade.clone(); }
        }
    }
    result
}
...
pub fn get_item_info(item_id: u64, bonus_ids: Option<&[u64]>) -> Option<Value> {
    ...
    if let Some(bids) = bonus_ids {
        ...
        if let Some(q) = resolved.get("quality").and_then(|q| q.as_u64()) { quality = q; }
        if let Some(i) = resolved.get("ilevel").and_then(|i| i.as_u64()) { ilevel = i; }
        if let Some(t) = resolved.get("tag").and_then(|t| t.as_str()) { tag = t.to_string(); }
        if let Some(s) = resolved.get("sockets").and_then(|s| s.as_u64()) { sockets = s; }
        if let Some(u) = resolved.get("upgrade").and_then(|u| u.as_str()) { upgrade = u.to_string(); }
    }
...
pub fn get_enchant_info(enchant_id: u64) -> Option<Value> {
    ...
    let name = enchant.get("itemName").or_else(|| enchant.get("displayName"))
...
pub fn get_gem_info(gem_item_id: u64) -> Option<Value> {
    ...
    let name = gem.get("itemName").or_else(|| gem.get("displayName"))
        .and_then(|n| n.as_str()).unwrap_or("");
    let icon = gem.get("itemIcon").or_else(|| gem.get("spellIcon"))
        .and_then(|i| i.as_str()).unwrap_or("");
    let quality = gem.get("quality").and_then(|q| q.as_u64()).unwrap_or(3);
    Some(serde_json::json!({ "gem_id": gem_item_id, "name": name, "icon": icon, "quality": quality }))
}

pub fn get_item_armor_subclass(item_id: u64) -> Option<u64> {
    let item = items().get(&item_id)?;
    let item_class = item.get("itemClass")?.as_u64()?;
    if item_class != 4 { return None; }
...
pub fn upgrade_bonus_ids_to_max(bonus_ids: &[u64]) -> Vec<u64> {
    let um = upgrade_max();
    bonus_ids.iter().map(|bid| *um.get(bid).unwrap_or(bid)).collect()
}
...
pub fn upgrade_simc_input(simc_input: &str) -> String {
    ...
    re.replace_all(simc_input, |caps: &regex::Captures| {
        ...
        let ids: Vec<u64> = raw.split(&['/', ':'][..]).filter_map(|s| s.parse().ok()).collect();
        let upgraded = upgrade_bonus_ids_to_max(&ids);
        format!("bonus_id={}", upgraded.iter().map(|id| id.to_string()).collect::<Vec<_>>().join(sep))
    }).to_string()
}
...
pub fn upgrade_items_by_slot(
    ...
    for (slot, slot_items) in items_by_slot {
        let new_items: Vec<Value> = slot_items.iter().map(|item| {
            let old_bonus_ids: Vec<u64> = item.get("bonus_ids")
                .and_then(|b| b.as_array())
                .map(|arr| arr.iter().filter_map(|v| v.as_u64()).collect())
                .unwrap_or_default();
            let new_bonus_ids = upgrade_bonus_ids_to_max(&old_bonus_ids);
            if new_bonus_ids == old_bonus_ids { return item.clone(); }
            ...
            if let Some(simc) = item.get("simc_string").and_then(|s| s.as_str()) {
                let new_simc = bonus_re.replace(simc, |caps: &regex::Captures| {
                    let raw = &caps[1];
                    let sep = if raw.contains('/') { "/" } else { ":" };
                    format!("bonus_id={}", new_bonus_ids.iter().map(|id| id.to_string()).collect::<Vec<_>>().join(sep))
                }).to_string();
...
pub fn apply_copy_enchants(
    ...
    for (slot, slot_items) in items_by_slot {
        let equipped = slot_items.iter().find(|it| it.get("is_equipped").and_then(|v| v.as_bool()).unwrap_or(false));
        let ench_id = equipped.and_then(|e| e.get("enchant_id")).and_then(|e| e.as_u64()).unwrap_or(0);
        ...
        let new_items: Vec<Value> = slot_items.iter().map(|item| {
            let is_equipped = item.get("is_equipped").and_then(|v| v.as_bool()).unwrap_or(false);
            let current_ench = item.get("enchant_id").and_then(|e| e.as_u64()).unwrap_or(0);
            if is_equipped || current_ench == ench_id { return item.clone(); }

            let mut updated = item.clone();
            updated["enchant_id"] = serde_json::json!(ench_id);
            if let Some(simc) = item.get("simc_string").and_then(|s| s.as_str()) {
                let new_simc = if re.is_match(simc) {
                    re.replace(simc, &format!("enchant_id={}", ench_id)).to_string()
                } else {
                    id_re.replace(simc, &format!("$1,enchant_id={}", ench_id)).to_string()
                };
                updated["simc_string"] = serde_json::json!(new_simc);
            }
            updated
...
pub fn dungeon_normal_ilvl() -> u64 {
    season_cfg().get("dungeonNormal").and_then(|d| d.get("ilvl")).and_then(|v| v.as_u64()).unwrap_or(0)
}

pub fn dungeon_normal_quality() -> u64 {
    season_cfg().get("dungeonNormal").and_then(|d| d.get("quality")).and_then(|v| v.as_u64()).unwrap_or(3)
}
