...
export default function TopGearPage() {
  const { simcInput, fightStyle, threads, maxCombinations, selectedTalent, targetCount, fightLength, customApl, simcHeader, simcBasePlayer, simcRaidActors, simcPostCombos, simcFooter, scenarios, clearScenarios } = useSimContext();
  ...
  return (
    <div className="space-y-6">
      ...
      >
        ...
        ) : scenarios.length > 0 ? `Run ${scenarios.length} Scenario${scenarios.length > 1 ? "s" : ""}` : "Find Top Gear"}
      </button>
      ...
      >
        ...
        <span className="whitespace-nowrap max-w-0 group-hover:max-w-[10rem] overflow-hidden transition-all duration-200 opacity-0 group-hover:opacity-100">
          {submitting ? "Starting sim…" : scenarios.length > 0 ? `Run ${scenarios.length} Scenario${scenarios.length > 1 ? "s" : ""}` : "Find Top Gear"}
        </span>
      </button>
