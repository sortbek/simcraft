        ...
        <div className="space-y-4">
          ...
          >
            {submitting ? (
              ...
              : scenarios.length > 0 ? `Run ${scenarios.length} Scenario${scenarios.length > 1 ? "s" : ""} (${selected.size} items)`
          ...
          >
            ...
            <span className="whitespace-nowrap max-w-0 group-hover:max-w-[10rem] overflow-hidden transition-all duration-200 opacity-0 group-hover:opacity-100">
              {submitting ? "Starting sim…" : scenarios.length > 0 ? `Run ${scenarios.length} Scenarios` : `Find Upgrades (${selected.size})`}
            </span>
          </button>
