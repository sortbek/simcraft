...
export default function Select<T>({ value, options, onChange, isEqual = Object.is }: SelectProps<T>) {
    ...
    <div ref={ref} className="relative">
      ...
      >
        <span className="flex items-center gap-2 truncate">
          ...
          {selected?.sublabel && (
            <span className="text-xs tabular-nums text-on-surface-variant">{selected.sublabel}</span>
          )}
        </span>
        <svg
