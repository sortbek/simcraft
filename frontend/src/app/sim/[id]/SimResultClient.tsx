...
import { getScenarioSiblings, formatScenarioLabel, type ScenarioSibling } from "../../lib/scenario-siblings";
      ...
      {siblings && siblings.length > 1 && (
        <div className="card p-3">
          <div className="flex items-center gap-2 flex-wrap">
            <span className="text-[11px] text-muted uppercase tracking-wider shrink-0">Scenarios</span>
            ...
            {siblings.map((s) => {
              ...
              return (
                <a
                  key={s.id}
                  href={`/sim/${s.id}`}
                  className={`px-2.5 py-1 rounded-lg text-[12px] font-medium transition-all border ${
                    isCurrent
